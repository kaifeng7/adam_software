...
{
    // costmap 以 laser 为中心
    ROS_INFO("123 %f",param_resolution_);
...
void pointsCB(const sensor_msgs::PointCloud2ConstPtr &msg)
{
    ...
    for (const auto &p : pc_laser.points)
    {
        ...
        double z = p.z;
        // TODO: 判断 z 不好
        if ((i_x < 0 || i_x >= g_width_) || (i_y < 0 || i_y >= g_height_) || (z < param_zl_ || z > param_zh_))
        {
            continue;
        }
    ...
    if (g_count_ % param_frame_num_ == 0)
    {
        ...
        for (auto &i : g_costmap_)
        {
            if (i > 0)
            {
                if (i >= param_frame_num_ * param_confidence_)
                {
                    i = 100;
                }
                else
        ...
        msg_global_costmap_.info = msg_costmap_.info;
        ...
        for (int li = 0; li < msg_costmap_.info.width/10; li++)
        {
            for (int lj = 0; lj < msg_costmap_.info.height/10; lj++)
            {
                ...
                gp = tf_m2l * lp;
                //ROS_INFO("%f %f %f",tf_m2l.getOrigin().getX(),tf_m2l.getOrigin().getY(),tf_m2l.getOrigin().getZ());
