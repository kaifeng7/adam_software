...
RayGroundFilter::RayGroundFilter(ros::NodeHandle &nh):nh(nh){}
...
void RayGroundFilter::run(){
    ...
    pub_global_ = nh.advertise<sensor_msgs::PointCloud2>("/global/points_no_ground",10);
}
...
void RayGroundFilter::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    ...
    //     tf::StampedTransform transform_base2laser;
    //     tf::TransformListener tf_listener;
    //     double param_tf_timeout_ = 1.0;
    //     std::string param_base_frame_ = "base_link";
    //     std::string param_laser_frame_ = "velodyne";

    //     try{
    //         ros::Time now = ros::Time::now();
    //         ROS_INFO("now: %f", now.toSec());
    //         tf_listener.waitForTransform(param_base_frame_, param_laser_frame_, ros::Time(0), ros::Duration(param_tf_timeout_ * 10), ros::Duration(param_tf_timeout_ / 3));
    //         tf_listener.lookupTransform(param_base_frame_, param_laser_frame_, ros::Time(0), transform_base2laser);
    //     }
    //     catch (const tf::TransformException &ex){
    //         ROS_ERROR("Error waiting for tf in init: %s", ex.what());
    //         // return false;
    //     }
    //     Eigen::Translation3f tf_base2laser(transform_base2laser.getOrigin().getX(), transform_base2laser.getOrigin().getY(), transform_base2laser.getOrigin().getZ());
    //     double roll, pitch, yaw;
    //     tf::Matrix3x3(transform_base2laser.getRotation()).getEulerYPR(yaw, pitch, roll);
    //     Eigen::AngleAxisf rot_x_btol(roll, Eigen::Vector3f::UnitX());
    //     Eigen::AngleAxisf rot_y_btol(pitch, Eigen::Vector3f::UnitY());
    //     Eigen::AngleAxisf rot_z_btol(yaw, Eigen::Vector3f::UnitZ());
    //     Eigen::Matrix4f matrix_base2laser = (tf_base2laser * rot_z_btol * rot_y_btol * rot_x_btol).matrix();

    //     try{
    //         ros::Time now = ros::Time::now();
    //         ROS_INFO("now: %f", now.toSec());
    //         tf_listener.waitForTransform("map", "base_link", ros::Time(0), ros::Duration(param_tf_timeout_ * 10), ros::Duration(param_tf_timeout_ / 3));
    //         tf_listener.lookupTransform("map", "base_link", ros::Time(0), transform_map2base);
    //     }
    //     catch (const tf::TransformException &ex){
    //         ROS_ERROR("Error waiting for tf in init: %s", ex.what());
    //         // return false;
    ...
    //     Eigen::AngleAxisf rot_y_btol(pitch, Eigen::Vector3f::UnitY());
    //     Eigen::AngleAxisf rot_z_btol(yaw, Eigen::Vector3f::UnitZ());
    //     Eigen::Matrix4f matrix_map2base = (tf_base2laser * rot_z_btol * rot_y_btol * rot_x_btol).matrix();

    //     Eigen::Matrix4f final_tf = matrix_map2base*matrix_base2laser;
    //     pcl::PointCloud<pcl::PointXYZ>::Ptr global_noground_pc;
    //     pcl::transformPointCloud(*no_ground_cloud_ptr,*global_noground_pc,final_tf);
