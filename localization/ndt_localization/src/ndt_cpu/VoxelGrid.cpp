...
VoxelGrid<PointSourceType>::VoxelGrid() : voxel_num_(0),
										  max_x_(FLT_MIN),
										  max_y_(FLT_MIN),
										  max_z_(FLT_MIN),
										  min_x_(FLT_MAX),
...
// template <typename PointSourceType>
// void VoxelGrid<PointSourceType>::writeCCMap(const std::string &file_path)
// {
// 	// step 1. write map params
// 	std::string mp_path = file_path.substr(0, file_path.find_last_of('.')) + ".yaml";
// 	std::ofstream mp_file(mp_path);
// 	// mp_file.open(mp_path);
// 	mp_file << "voxel_num: " << voxel_num_ << std::endl;
// 	mp_file << "max_coordinate: [" << max_x_ << ", " << max_y_ << ", " << max_z_ << "]" << std::endl;
// 	mp_file << "min_coordinate: [" << min_x_ << ", " << min_y_ << ", " << min_z_ << "]" << std::endl;
// 	mp_file << "voxel: [" << voxel_x_ << ", " << voxel_y_ << ", " << voxel_z_ << "]" << std::endl;
// 	mp_file << "max_bound: [" << max_b_x_ << ", " << max_b_y_ << ", " << max_b_z_ << "]" << std::endl;
// 	mp_file << "min_bound: [" << min_b_x_ << ", " << min_b_y_ << ", " << min_b_z_ << "]" << std::endl;
// 	mp_file << "vgrid: [" << vgrid_x_ << ", " << vgrid_y_ << ", " << vgrid_z_ << "]" << std::endl;
// 	mp_file << "real_max_bound: [" << real_max_bx_ << ", " << real_max_by_ << ", " << real_max_bz_ << "]" << std::endl;
// 	mp_file << "real_min_bound: [" << real_min_bx_ << ", " << real_min_by_ << ", " << real_min_bz_ << "]" << std::endl;

// 	// step 2. write centroid and icovariance of each voxel
// 	pcl::PointCloud<VoxelCentoridCovariance> cc_data;
// 	cc_data.width = voxel_num_;
// 	cc_data.height = 1;
// 	cc_data.is_dense = false;
// 	cc_data.points.resize(cc_data.width * cc_data.height);

// 	for (int vid = 0; vid < voxel_num_; ++vid)
// 	{
// 		for (int j = 0; j < 6; j++)
// 		{
// 			// const Eigen::Vector3d &centroid_vid = centroid_[vid];
// 			// cc_data.points[vid].centroid[j] = centroid_vid(j);
// 			cc_data.points[vid].centroid[j] = (*centroid_)[vid](j);
// 		}

// 		for(int j = 0; j < 36; j++){
// 			// const Eigen::Matrix3d &icovariance_vid = icovariance_[vid];
// 			// cc_data.points[vid].iCovariance[j] = icovariance_vid(j);
// 			cc_data.points[vid].iCovariance[j] = (*icovariance_)[vid](j);
// 		}
// 	}

// 	pcl::io::savePCDFileASCII(file_path, cc_data);
// }

// template <typename PointSourceType>
// void VoxelGrid<PointSourceType>::readCCMap(const std::string &file_path)
// {

// 	std::string mp_path = file_path.substr(0, file_path.find_last_of('.')) + ".yaml";
// 	YAML::Node mp_yaml = YAML::LoadFile(file_path);
// 	voxel_num_ = mp_yaml["voxel_num"].as<int>();
	
// 	std::vector<float> max_coordinate = mp_yaml["max_coordinate"].as<std::vector<float>>();
// 	std::vector<float> min_coordinate = mp_yaml["min_coordinate"].as<std::vector<float>>();
// 	std::vector<float> voxel = mp_yaml["voxel"].as<std::vector<float>>();
// 	std::vector<int> max_bound = mp_yaml["max_bound"].as<std::vector<int>>();
// 	std::vector<int> min_bound = mp_yaml["min_bound"].as<std::vector<int>>();
// 	std::vector<int> vgrid = mp_yaml["vgrid"].as<std::vector<int>>();
// 	std::vector<int> real_max_bound = mp_yaml["real_max_bound"].as<std::vector<int>>();
// 	std::vector<int> real_min_bound = mp_yaml["real_min_bound"].as<std::vector<int>>();

// 	max_x_ = max_coordinate[0];
// 	max_y_ = max_coordinate[1];
// 	max_z_ = max_coordinate[2];
// 	min_x_ = min_coordinate[0];
// 	min_y_ = min_coordinate[1];
// 	min_z_ = min_coordinate[2];
// 	voxel_x_ = voxel[0];
// 	voxel_y_ = voxel[1];
// 	voxel_z_ = voxel[2];
// 	max_b_x_ = max_bound[0];
// 	max_b_y_ = max_bound[1];
// 	max_b_z_ = max_bound[2];
// 	min_b_x_ = min_bound[0];
// 	min_b_y_ = min_bound[1];
// 	min_b_z_ = min_bound[2];
// 	vgrid_x_ = vgrid[0];
// 	vgrid_y_ = vgrid[1];
// 	vgrid_z_ = vgrid[2];
// 	real_max_bx_ = real_max_bound[0];
// 	real_max_by_ = real_max_bound[1];
// 	real_max_bz_ = real_max_bound[2];
// 	real_min_bx_ = real_min_bound[0];
// 	real_min_by_ = real_min_bound[1];
// 	real_min_bz_ = real_min_bound[2];
...
// template <typename PointSourceType>
// void VoxelGrid<PointSourceType>::setCCInput(typename pcl::PointCloud<PointSourceType>::Ptr input_cloud, const std::string &ccmap_path)
// {
// 	if (input_cloud->points.size() > 0)
// 	{
// 		/* If no voxel grid was created, then
