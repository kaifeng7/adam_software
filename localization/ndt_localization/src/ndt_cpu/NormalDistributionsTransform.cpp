...
double NormalDistributionsTransform<PointSourceType,
                                    PointTargetType>::getStepSize() const
{
    return step_size_;
}
...
// template <typename PointSourceType, typename PointTargetType>
// void NormalDistributionsTransform<PointSourceType, PointTargetType>::
//     setCCInputTarget(typename pcl::PointCloud<PointTargetType>::Ptr input, const std::string &ccmap_path)
// {
//     Registration<PointSourceType, PointTargetType>::setInputTarget(input);
...
//     {
//         voxel_grid_.setLeafSize(resolution_, resolution_, resolution_);
//         voxel_grid_.setCCInput(input, ccmap_path);
//     }
// }
// template <typename PointSourceType, typename PointTargetType>
// void NormalDistributionsTransform<PointSourceType, PointTargetType>::
//     generateCCMap(const std::string &ccmap_path)
// {
//     voxel_grid_.writeCCMap(ccmap_path);
// }

// template <typename PointSourceType, typename PointTargetType>
// void NormalDistributionsTransform<PointSourceType, PointTargetType>::loadCCMap(const std::string &mapfile)
// {
//     voxel_grid_.readCCMap(mapfile);
// }
    ...
    computeTransformation(const Eigen::Matrix<float, 4, 4> &guess)
{
    ...
    // 这个 score 不知道含义是什么。计算 score function 关于 位姿 p 的 Jacobian 和
    // Hessian
                       ...
                       Eigen::Matrix<double, 6, 1> pose, bool compute_hessian)
{
    ...
    for (int idx = 0; idx < source_cloud_->points.size(); idx++)
    {
        ...
        // TODO: 为什么不直接找最近的 voxel，而是找一个 radius 内的所有 voxel 来更新
        // J,H。Magnusson 的论文里没有直接说明，但在 6.3.5 介绍 linked cells
        // 时简单说了一下不同，应该是为了平滑
